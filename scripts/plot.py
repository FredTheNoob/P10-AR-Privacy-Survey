...
def censoring_scenario_question(df: pd.DataFrame):
    palette = {
        "AI answered": "crimson",
...
def censoring_question(df: pd.DataFrame):
    palette = {
        "AI answered": "crimson",
        "Question 1": "steelblue",
        "Question 2": "darkorange",
    ...
    # Compute averages grouped by scenario, censoringmethod, and question
    grouped = (
        df.groupby(["censoringMethod", "question"])["answer_numeric"]
        .mean()
        .reset_index()
    )
