...
def run_analysis(data, question):
    ...
    # 5. Execute Friedman Test
    stat, p = friedmanchisquare(pivot_df['NONE'], 
                                pivot_df['BLUR'], 
                                pivot_df['BLACK_BOX'], 
    ...
    # 6. Post-hoc Analysis (Only if Friedman is significant)
    if p < 0.05:
        print("\nSignificant difference found. Running Post-hoc Wilcoxon Tests...")
        pairs = list(combinations(methods, 2))
        
        # Bonferroni Correction: Alpha (0.05) / number of comparisons (6)
        ...
        for m1, m2 in pairs:
            ...
            stat_w, p_w = wilcoxon(pivot_df[m1], pivot_df[m2])
            is_significant = "YES" if p_w < bonferroni_alpha else "no"
            results.append({
